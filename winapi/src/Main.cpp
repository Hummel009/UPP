...
канст вектар<цэл> фактарыял = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600};

НАКНО поле;
вектар<страка> даныя;

пуст зарэгКнопку(НАКНО акно, цэл нум, страка тэкст, цэл хСеткі, цэл уСеткі);
НАКНО зарэгПоле(НАКНО акно);
пуст штурхСімвалА(страка сімвал);
пуст штурхАперацыю(страка аперацыя);
пуст вылічыцьА();

LRESULT працэсАкна(НАКНО акно, БЦЭЛ сігнал, ЎПАРАМ ўПарам, ЛПАРАМ лПарам)
{
...
пуст вылічыць()
{
	...
	калі(даныя.size() == 2)
	{
		...
		калі(аператары1.contains(аператар))
		{
		...
		інакш калі(аператары2.contains(аператар))
		{
...
пуст штурхСімвал(страка сімвал);
пуст штурхСімвалА(страка сімвал)
{
	аўта буфер = унікальны<сімв[]>(СТАНДАРТНАЯ_ЁМІСТНАСЦЬ);
	GetWindowTextA(поле, буфер.get(), СТАНДАРТНАЯ_ЁМІСТНАСЦЬ);
	...
	інакш калі(сімвал == ".")
	{
		калі(стр.find(".") == страка::npos && !стр.empty())
		{
			штурхСімвал(сімвал);
		}
	}
	інакш
	{
		...
		інакш
		{
			...
			калі(!аператары.contains(аператар))
			{
				штурхСімвал(сімвал);
			}
		}
