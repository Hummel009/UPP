...
вобласць беларусь
{
	...
	{
	публіч:
		пералік Месяц{СТУ, ЛЮТ, САК, КРА, ТРА, ЧЭР, ЛІП, ЖНІ, ВЕР, КАС, ЛІС, СНЕ};

		пералік ДзеньНядзелі{НЯД, ПАН, АЎТ, СЕР, ЧАЦ, ПЯТ, СУБ};
		...
		стат пуст запуск()
		{
			...
			цэл рэзультат = дата + днёўПасляНГ[месяц] + (год - 1) * 365;
			цэл дапаўненне = (год - 1) / 4 - (год - 1) / 100 + (год - 1) / 400;
			калі (высакосны && (днёўПасляНГ[месяц]) + дата > 59)
			{
				дапаўненне += 1;
			}
			рэзультат += дапаўненне;
			цэл дзень = рэзультат % 7;
			ДзеньНядзелі дзеньНядзелі = стат_прывядз<ДзеньНядзелі>(дзень);
			вывад << дзеньНядзелі << абзац;
		}
	};
}
